...
class FloattingScreen extends StatefulWidget {
  const FloattingScreen({super.key});

  @override
  State<FloattingScreen> createState() => _FloattingScreenState();
}

class _FloattingScreenState extends State<FloattingScreen> {
    ...
    if (Placeid.toString() != "" &&
      ...
      if (response.statusCode == 200) {
        ...
        if (resposne['msg'].toString().contains("success")) {
          Navigator.push(
              context, MaterialPageRoute(builder: (context) => MainScreen(selectedIndex: 1,)));
